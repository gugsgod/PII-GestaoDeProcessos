...
Future<Response> onRequest(RequestContext context) async {
  ...
  final where = <String>['es.qt_disp < es.minimo'];
  ...
  try {
    ...
    final rows = await conn.execute(
      Sql.named('''
        SELECT
          ...
          m.id, m.cod_sap, m.descricao, m.unidade,
          lf.id,
          lf.contexto::text AS contexto,
          lf.base_id, lf.veiculo_id, lf.nome
        FROM estoque_saldos es
        JOIN materiais m     ON m.id = es.material_id
        JOIN locais_fisicos lf ON lf.id = es.local_id
        $whereSql
        ORDER BY (es.minimo - es.qt_disp) DESC, m.cod_sap
    ...
    final data = rows
        .map((r) => {
              ...
              'qt_disp': r[2],
              'minimo': r[3],
              'deficit': (r[3] as double) - (r[2] as double),
              'material': {
                'id': r[4],
                'cod_sap': r[5],
                'descricao': r[6],
    ...
    print('GET /estoque/minimos error: $e\n$st');
