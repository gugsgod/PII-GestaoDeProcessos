...
Future<Response> onRequest(RequestContext context) async {
  ...
  final operacao = qp['tipo']?.trim(); // entrada|saida|transferencia
  ...
  if (operacao != null && operacao.isNotEmpty) {
    where.add('mv.tipo = @tipo');
    params['tipo'] = operacao;
  }
  ...
  try {
    ...
    final rows = await conn.execute(
      Sql.named('''
        SELECT
          mv.id, mv.operacao, mv.material_id,
          mv.origem_local_id, mv.destino_local_id,
          mv.lote, mv.quantidade::float8 AS quantidade,
          mv.responsavel_id, mv.observacao,
          mv.created_at,
          m.cod_sap, m.descricao, m.unidade
